...
namespace MasterControl
{
    public class Util
    {
        ...
        public static string SendNotification(string deviceId, string message)
        {
            ...
            var value = message;
            WebRequest tRequest;
            if (MasterControlSettings.Instance.debug) Logging.Write(string.Format("[bC]: toAndroid start "));
            ...
            try
            {
                ...
                dataStream.Close();
                tResponse.Close();
                if (MasterControlSettings.Instance.debug) Logging.Write(string.Format("[bC]: toAndroid {0} ", sResponseFromServer));
            ...
            catch (WebException e)
            {
                ...
                Logging.Write(string.Format("[bC]: Fehler: {0}", responseString));
                return "";
            }
            catch (Exception e)
            {
                Logging.Write(string.Format("[bC]: Fehler: {0}", e.Message));
                return "";
            }
        }
        ...
        {
            if (MasterControlSettings.Instance.debug)
                using (StreamWriter w = File.AppendText("log.txt"))
                {
                    Logging.Write(string.Format("[bC]: Fehler: {0}", Styx.StyxWoW.Me.IsOutOfBounds));
                    w.Write("\r\nLog Entry : ");
        ...
        public static string PostToFtp(string imagFilePath)
        {
            ...
            try
            {
                ...
                string url = MasterControlSettings.Instance.scripturl;
                if (!url.StartsWith("http://")) url = "http://" + url;
                Logging.Write(string.Format("[bC]: url: {0}", url));
        ...
        public static string HttpPost(string url, string[] paramName, string[] paramVal)
        {
            ...
            catch (WebException e)
            {
                ...
                string responseString = responseReader.ReadToEnd();
                if (MasterControlSettings.Instance.debug) Logging.Write(string.Format("[bC]: Fehler: {0}", responseString));
                return "";
            }
        ...
        {
            if (MasterControlSettings.Instance.prowlapi != "")
            {
                string[] sendParam = { "apikey", "application", "event", "description", "priority" };
                string[] sendVal = { MasterControlSettings.Instance.prowlapi, "BuddyCon", header, message + " Bot: " + name + " - " + server, "1" };
                string response = Util.HttpPost("https://api.prowlapp.com/publicapi/add", sendParam, sendVal);
                //Logging.Write(string.Format("[bC]: Prowl {0} ", response));
